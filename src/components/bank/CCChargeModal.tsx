        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-baseline gap-3">
            ...
            {!isLinkMode && bankTransaction && (() => {
              ...
              if (!matchedCard && bankTransaction.linked_credit_card_id) {
                matchedCard = creditCards.find(c => c.id === bankTransaction.linked_credit_card_id) || null
              }
        ...
        {isLinkMode ? (
          ...
          <>
            ...
            <div className="mb-6 grid grid-cols-3 gap-4">
              {/* Date Range KPI */}
              <div className="p-4 bg-background/50 border border-text-muted/20 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <CalendarDaysIcon className="w-4 h-4 text-text-muted" />
                  <span className="text-xs text-text-muted font-medium uppercase tracking-wide">Date Range</span>
                </div>
              ...
              <div className="p-4 bg-background/50 border border-text-muted/20 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-text-muted font-medium uppercase tracking-wide">Match Amount</span>
                </div>
                <div className="text-lg font-semibold text-text">
                  {formatShekel(ccAmount)} / {formatShekel(Math.abs(bankAmount))}
                </div>
              ...
              <div className="p-4 bg-background/50 border border-text-muted/20 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <CreditCardIcon className="w-4 h-4 text-text-muted" />
                  <span className="text-xs text-text-muted font-medium uppercase tracking-wide">CC Transactions</span>
                </div>
                <div className="text-lg font-semibold text-text">
                  {ccCount}
                </div>
            ...
            {/* CC Transactions table - only show if there are linked transactions */}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-text-muted/20">
                        ...
                        <th className="text-start py-2 px-2 text-text-muted font-medium">Date</th>
                    ...
                    <tbody>
                      {matchResult.cc_transactions.map((tx) => (
                        <tr key={tx.id} className="border-b border-text-muted/10 hover:bg-background/30">
                          ...
                          <td className="py-2 px-2 text-text text-end">
                ...
                {selectedIds.size > 0 && (
                  <div className="mb-4">
                    <button
                      ...
                      className="w-full px-4 py-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                    >
                      ...
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-surface">
                          <tr className="border-b border-text-muted/20">
                            ...
                            <th className="text-start py-2 px-2 text-text-muted font-medium">
                              {attachDateField === 'transaction_date' ? 'Purchase' : 'Billing'}
                            </th>
                        ...
                        <tbody>
                          {filteredCCTxs.map((tx) => {
                            ...
                            const displayDate = attachDateField === 'transaction_date' ? tx.transaction_date : tx.charge_date
                            return (
                              <tr key={tx.id} className="border-b border-text-muted/10 hover:bg-background/30">
                                <td className="py-2 px-2 text-center">
                                  <input
