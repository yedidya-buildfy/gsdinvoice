...
export function BankTab({ onRefetch }: BankTabProps) {
  const { user } = useAuth()
  ...
  // Filter to only bank_regular transactions
  const bankTransactions = useMemo(() => {
    return transactions.filter((tx) =>
      tx.transaction_type === TRANSACTION_TYPE.BANK_REGULAR ||
      (!tx.transaction_type && !tx.is_credit_card_charge)
    )
  }, [transactions])

  // Fetch link counts for transactions
  const transactionIds = useMemo(() => bankTransactions.map(tx => tx.id), [bankTransactions])
  const { linkCounts, refetch: refetchLinkCounts } = useTransactionLinkCounts({
    transactionIds,
    enabled: bankTransactions.length > 0,
  })
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[BankTab] Delete failed:', error)
        return
      }

      console.log('[BankTab] Deleted', data, 'transactions')
      setSelectedIds(new Set())
      refetch()
      onRefetch?.()
    } catch (err) {
      console.error('[BankTab] Delete error:', err)
    } finally {
      setIsDeleting(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-background rounded-lg p-4 border border-border">
        <h3 className="text-sm font-medium text-text mb-3">Import Bank Statement</h3>
