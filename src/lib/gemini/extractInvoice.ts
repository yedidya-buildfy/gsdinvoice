/**
 * Client-side Gemini API integration for invoice extraction
 * Uses Gemini with code execution for reliable parsing of CSV/XLSX files
 */

import * as XLSX from 'xlsx'
import type { InvoiceExtraction } from '@/lib/extraction/types'
...
export async function extractInvoiceWithGemini(
  ...
  // Build the request based on file type
  let requestBody: {
  ...
  if (isSpreadsheetType(fileType)) {
    ...
    if (fileType.toLowerCase() === 'csv') {
      ...
      const sheets = await xlsxToCsvSheets(fileBlob)
      console.log(
    ...
    // Convert UTF-8 string to base64 (modern approach, no deprecated unescape)
    ...
    if (fileBlob.size > MAX_SIZE) {
      throw new Error(
        `File too large: ${Math.round(fileBlob.size / 1024 / 1024)}MB. Maximum is 20MB.`
      )
    }
...
export type { InvoiceExtraction, ExtractedLineItem } from '@/lib/extraction/types'
